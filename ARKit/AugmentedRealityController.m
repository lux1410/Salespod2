...
- (void)updateLocations
{
    ...
    NSMutableArray* arr=[[NSMutableArray alloc] initWithCapacity:[[self coordinates] count]];
    ...
    if ([arr count]>1){
        ...
        arr=[[arr sortedArrayUsingComparator:^NSComparisonResult(id s1, id s2) {
    ...
    for (ARGeoCoordinate *item in [self coordinates]) {
		...
		if ([self shouldDisplayCoordinate:item]) {
           ...
           int idx= [arr indexOfObject:item];
